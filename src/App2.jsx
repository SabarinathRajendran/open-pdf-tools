...
const PDFCropper = () => {
  ...
  const handleCrop = async () => {
    ...
    try {
      ...
      // The crop area is in rendered canvas coordinates, we need to convert to PDF coordinates
      const pdfCropBox = {
        x: cropArea.x / renderScale,
        y: (currentViewport.height - cropArea.y - cropArea.height) / renderScale, // Flip Y and scale
        width: cropArea.width / renderScale,
        height: cropArea.height / renderScale
      };
      ...
      for (const pageNum of pagesToCrop) {
        ...
        // Calculate crop bounds ensuring they're within page bounds
        const cropLeft = Math.max(0, pdfCropBox.x);
        const cropBottom = Math.max(0, pdfCropBox.y);
        const cropRight = Math.min(pageWidth, pdfCropBox.x + pdfCropBox.width);
        const cropTop = Math.min(pageHeight, pdfCropBox.y + pdfCropBox.height);
        
        // Ensure we have valid dimensions
        if (cropRight > cropLeft && cropTop > cropBottom) {
          // Set the crop box for the page
          originalPage.setCropBox(cropLeft, cropBottom, cropRight, cropTop);
          
          // Also set media box to match crop box for better compatibility
          originalPage.setMediaBox(cropLeft, cropBottom, cropRight, cropTop);
        }
